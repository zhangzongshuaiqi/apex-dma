...
//  [X] Renderer: User texture binding. Use 'ID3D11ShaderResourceView*' as ImTextureID. Read the FAQ
//  about ImTextureID! [X] Renderer: Large meshes support (64k+ vertices) with 16-bit indices.
...
//  multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX11: Replaced
//  direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-02-18: DirectX11: Change blending equation to preserve alpha in output buffer.
//  2019-08-01: DirectX11: Fixed code querying the Geometry Shader state (would generally error with
//  Debug layer enabled). 2019-07-21: DirectX11: Backup, clear and restore Geometry Shader is any is
//  bound when calling ImGui_ImplDX10_RenderDrawData. Clearing Hull/Domain/Compute shaders without
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data) {
  ...
  // viewport apps.
  {
    ...
    if (ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) !=
        S_OK)
      return;
