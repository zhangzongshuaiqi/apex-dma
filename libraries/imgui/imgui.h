...
// - Gallery .................... https://github.com/ocornut/imgui/issues/6897 (please post your
...
// (MSVC provides an equivalent mechanism via SAL Annotations but it would require the macros in a
// different
...
// cast from/to our preferred type. Add '#define IMGUI_DEFINE_MATH_OPERATORS' in your imconfig.h
// file to benefit from courtesy maths operators for those types.
...
// false".
//   This API is backward compatible with old code, as we guarantee that ImGuiChildFlags_Border ==
...
IMGUI_API ImVec2
GetWindowPos();  // get current window position in screen space (note: it is unlikely you need to
                 // use this. Consider using current layout pos instead, GetCursorScreenPos())
IMGUI_API ImVec2
GetWindowSize();  // get current window size (note: it is unlikely you need to use this. Consider
...
// useful to convert from window-local to absolute coordinates.
IMGUI_API ImVec2 GetCursorScreenPos();  // cursor position in absolute coordinates (prefer using
                                        // this, also more useful to work with ImDrawList API).
IMGUI_API void SetCursorScreenPos(const ImVec2& pos);  // cursor position in absolute coordinates
IMGUI_API ImVec2 GetCursorPos();  // [window-local] cursor position in window coordinates (relative
                                  // to window position)
...
// - A tooltip window can contain items of any types. SetTooltip() is a shortcut for the 'if
// (BeginTooltip()) { Text(...); EndTooltip(); }' idiom.
...
IMGUI_API void SetItemTooltip(const char* fmt, ...)
    IM_FMTARGS(1);  // set a text-only tooltip if preceeding item was hovered. override any previous
                    // call to SetTooltip().
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 23,  // [BETA] On child window: share focus scope, allow gamepad/keyboard navigation to
...
// (Legacy: bit 0 must always correspond to ImGuiChildFlags_Border to be backward compatible with
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_Border = 1 << 0,  // Show an outer border and enable WindowPadding. (IMPORTANT:
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_FramePadding =
      1
      << 10,  // Use FramePadding (even for an unframed text node) to vertically align text baseline
              // to regular widget height. Equivalent to calling AlignTextToFramePadding().
  ...
  ImGuiTreeNodeFlags_SpanAllColumns = 1 << 13,  // Frame will span all columns of its container
                                                // table (text will still fit in current column)
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
      1 << 14,  // (WIP) Nav: left direction may move to this TreeNode() from any of its child
                // (items submitted between TreeNode and TreePop)
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 15,  // FIXME: TODO: Disable automatic scroll on
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_NoNavOverride = 1
                                    << 11,  // IsItemHovered() only: Disable using gamepad/keyboard
...
// The keyboard key enum values are named after the keys on a standard US keyboard, and on other
...
enum ImGuiKey : int {
  ...
  // ACTION (download controller mapping PNG/PSD at http://dearimgui.com/controls_sheets)
  ...
  // [Internal] Prior to 1.87 we required user to fill io.KeysDown[512] using their own native index
  // + the io.KeyMap[] array. We are ditching this method but keeping a legacy path for user code
  // doing e.g. IsKeyPressed(MY_NATIVE_KEY_CODE) If you need to iterate all keys (for e.g. an input
  // mapper) you may use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NoMouse =
      1 << 4,  // Instruct imgui to clear mouse position/buttons in NewFrame(). This allows ignoring
...
enum ImGuiTableColumnFlags_ {
  ...
  ImGuiTableColumnFlags_NoHeaderLabel =
      1
      << 12,  // TableHeadersRow() will not submit horizontal label for this column. Convenient for
              // some small columns. Name will still appear in context menu or in angled headers.
...
struct ImGuiStyle {
  ...
  float TabMinWidthForCloseButton;  // Minimum width for close button to appear on an unselected tab
  ...
  ImVec2
      DisplayWindowPadding;  // Window position are clamped to be visible within the display area or
                             // monitors by at least this amount. Only applies to regular windows.
  ImVec2
      DisplaySafeAreaPadding;  // If you cannot see the edges of your screen (e.g. on a TV) increase
                               // the safe area padding. Apply to popups/tooltips as well regular
                               // windows. NB: Prefer configuring your TV sets correctly!
...
struct ImGuiIO {
  ...
  ImWchar PlatformLocaleDecimalPoint;  // '.'              // [Experimental] Configure decimal point
                                       // e.g. '.' or ',' useful for some languages (e.g. German),
  ...
  IMGUI_API void ClearInputKeys();    // Clear current keyboard/mouse/gamepad state + current frame
  ...
  // ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get automatic IME cursor
  // positioning.
#endif
...
struct ImGuiListClipper {
  ...
  inline void ForceDisplayRangeByIndices(int item_begin, int item_end) {
    IncludeItemsByIndex(item_begin, item_end);
     ...
     // inline ImGuiListClipper(int items_count, float items_height = -1.0f) { memset(this, 0,
...
struct ImDrawCmd {
  ...
  // always same as 'TextureId' (we will change this function for an upcoming feature)
...
struct ImDrawList {
  ...
  // If you want to create ImDrawList instances, pass them ImGui::GetDrawListSharedData() or create
  // and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)
  ...
  // a convenience fo user but not used by main library.
  ...
  // num_segments); } // OBSOLETED in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2&
  // p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0) { PathBezierCubicCurveTo(p2, p3,
...
struct ImFontConfig {
  ...
  int OversampleH;  // 2        // Rasterize at higher quality for sub-pixel positioning. Note the
  ...
  ImVec2 GlyphExtraSpacing;  // 0, 0     // Extra spacing (in pixels) between glyphs. Only X axis is
                             // supported for now.
  ...
  float RasterizerDensity;   // 1.0f     // DPI scale for rasterization, not altering other font
                             // metrics: make it easy to swap between e.g. a 100% and a 400% fonts
  // for a zooming display. IMPORTANT: If you increase this it is expected
  // that you increase font scale accordingly, otherwise quality may look
  // lowered.
...
struct ImFontAtlas {
  ...
  // compatibility, but note that unless you manually manipulate or copy color data into the texture
  // (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white
  ...
  unsigned char* TexPixelsAlpha8;  // 1 component per pixel, each component is unsigned 8-bit. Total
  ...
  const ImFontBuilderIO*
      FontBuilderIO;  // Opaque interface to a font builder (default to stb_truetype, can be changed
...
struct ImFont {
  ...
  int MetricsTotalSurface;  // 4     // out //            // Total surface in pixels to get an idea
                            // of the font rasterization/texture cost (not exact, we approximate the
                            // cost of padding between glyphs)
...
// static inline bool BeginChild(const char* str_id, const ImVec2& size_arg, bool border,
// ImGuiWindowFlags window_flags){ return BeginChild(str_id, size_arg, border ?
// ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags); } // Unnecessary as true ==
// ImGuiChildFlags_Border static inline bool BeginChild(ImGuiID id, const ImVec2& size_arg, bool
// border, ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg, border ?
// ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true ==
// ImGuiChildFlags_Border
...
IMGUI_API ImGuiKey GetKeyIndex(
    ImGuiKey key);  // Map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]. When
