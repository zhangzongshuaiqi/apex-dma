...
struct ImGuiNavTreeNodeData;  // Temporary storage for last TreeNode() being a Left arrow landing
                              // candidate.
...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_AllowOverlap = 1 << 9,  // false     // Allow being overlapped by another widget.
                                         // Not-hovered to Hovered transition deferred by a frame.

  // Controlled by widget code
  ImGuiItemFlags_Inputable =
      1 << 10,  // false     // [WIP] Auto-activate input mode when tab focused. Currently only used
...
// SetNextItemSelectionUserData()/BeginMultiSelect() (Most users are likely to use this store an
// item INDEX but this may be used to store a POINTER as well.)
...
struct ImGuiLastItemData {
  ...
  // ImGuiItemStatusFlags is set.
  ImRect
      DisplayRect;  // Display rectangle (ONLY VALID IF ImGuiItemStatusFlags_HasDisplayRect is set)
  ImRect ClipRect;  // Clip rectangle at the time of submitting item (ONLY VALID IF
...
// ImGuiTreeNodeFlags_NavLeftJumpsBackHere. This is the minimum amount of data that we need to
// perform the equivalent of NavApplyItemToResult() and which we can't infer in TreePop() Only
// stored when the node is a potential candidate for landing on a Left arrow jump.
...
enum ImGuiInputFlags_ {
  ...
  // In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable default, but it would
  // break too many behavior so everything is opt-in.
  ...
  // Locking is useful to make input-owner-aware code steal keys from non-input-owner-aware code. If
  // all code is input-owner-aware locking would never be necessary.
  ImGuiInputFlags_LockThisFrame =
      1 << 10,  // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
                // will NOT accepted for polling). Cleared at end of frame.
  ImGuiInputFlags_LockUntilRelease =
      1 << 11,  // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
                // will NOT accepted for polling). Cleared when the key is released or at end of
                // each frame if key is released.

  // Routing policies for Shortcut() + low-level SetShortcutRouting()
  // - The general idea is that several callers register interest in a shortcut, and only one owner
  // gets it.
  ...
  //   The whole system is order independent, so if Child1 does it calls before Parent results will
  //   be identical. This is an important property as it facilitate working with foreign code or
  //   larger codebase.
  // - Visualize registered routes in 'Metrics->Inputs' and submitted routes in 'Debug
  // Log->InputRouting'.
  // - When a policy (except for _RouteAlways *) is set, Shortcut() will register itself with
  // SetShortcutRouting(),
  //   allowing the system to decide where to route the input among other route-aware calls.
  //   (* Using ImGuiInputFlags_RouteAlways is roughly equivalent to calling
  //   IsKeyChordPressed(key)).
  // - Shortcut() uses ImGuiInputFlags_RouteFocused by default. Meaning that a Shortcut() call will
  // register
  //   a route and only succeed when parent window is in the focus-stack and if no-one with a higher
  //   priority is claiming the same shortcut.
  // - You can chain two unrelated windows in the focus stack using
  // SetWindowParentWindowForFocusRoute().
  // - Priorities: GlobalHigh > Focused (when owner is active item) > Global > Focused (when focused
  // window) > GlobalLow.
  // - Can select only 1 policy among all available.
  ImGuiInputFlags_RouteFocused =
      1 << 12,  // (Default) Honor focus route: Accept inputs if window is in focus stack. Deep-most
                // focused window takes inputs. ActiveId takes inputs over deep-most focused window.
  ImGuiInputFlags_RouteGlobalLow =
      1 << 13,  // Register route globally (lowest priority: unless a focused window or active item
  // registered the route) -> recommended Global priority IF you need a Global priority.
  ImGuiInputFlags_RouteGlobal =
      1 << 14,  // Register route globally (medium priority: unless an active item registered the
                // route, e.g. CTRL+A registered by InputText will take priority over this).
  ImGuiInputFlags_RouteGlobalHigh =
      1 << 15,  // Register route globally (higher priority: unlikely you need to use that: will
                // interfere with every active items, e.g. CTRL+A registered by InputText will be
                // overriden by this)
  ...
  ImGuiInputFlags_RouteUnlessBgFocused =
      1 << 17,  // Global routes will not be applied if underlying background/void is focused (== no
  ...
  ImGuiInputFlags_SupportedByShortcut = ImGuiInputFlags_RepeatMask_ | ImGuiInputFlags_RouteMask_ |
...
struct ImGuiNavItemData {
  ...
  ImGuiSelectionUserData
      SelectionUserData;  // I+Mov    // Best candidate SetNextItemSelectionData() value.
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImVec2
      BuildWorkOffsetMin;  // Work Area: Offset being built during current frame. Generally >= 0.0f.
  ImVec2
      BuildWorkOffsetMax;  // Work Area: Offset being built during current frame. Generally <= 0.0f.
...
struct ImGuiContext {
  ...
  bool ActiveIdUsingAllKeyboardKeys;  // Active widget will want to read all keyboard keys inputs.
                                      // (FIXME: This is a shortcut for not taking ownership of 100+
                                      // keys but perhaps best to not have the inconsistency)
  ImGuiKeyChord
  ...
  ImVector<ImGuiNavTreeNodeData>
      NavTreeNodeStack;  // Stack for TreeNode() when a NavLeft requested is emitted.
  ...
  // move/resize)
  ImGuiKeyChord
      ConfigNavWindowingKeyNext;  // = ImGuiMod_Ctrl | ImGuiKey_Tab, for reconfiguration (see #4828)
...
// incoming RowData sizeof() ~ 580 bytes + heap allocs described in TableBeginInitMemory()
...
// debugging/metrics. sizeof() ~ 120 bytes.
...
//   - Shortcut() submits a route then if currently can be routed calls IsKeyChordPressed() ->
//   function has (desirable) side-effects.
...
IMGUI_API int TypingSelectFindMatch(ImGuiTypingSelectRequest* req,
                                    ...
                                    void* user_data,
                                    int nav_item_idx);
IMGUI_API int TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest* req,
                                                  int items_count,
                                                  const char* (*get_item_name_func)(void*, int),
                                                  void* user_data,
                                                  int nav_item_idx);
IMGUI_API int TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest* req,
                                               int items_count,
                                               const char* (*get_item_name_func)(void*, int),
                                               void* user_data);
...
IMGUI_API void DebugAllocHook(ImGuiDebugAllocInfo* info,
                              int frame_count,
                              void* ptr,
