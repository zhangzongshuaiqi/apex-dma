...
// etc. And better standardize how widgets use 'GetColor32((held && hovered) ? ... : hovered ? ...)'
// vs 'GetColor32(held ? ... : hovered ? ...);' For mouse feedback we typically prefer the 'held &&
...
bool ImGui::InputTextEx(const char* label,
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf) {
    ...
    state->CurLenA =
        (int)(buf_end - buf);  // We can't get the result from ImStrncpy() above because it is not
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
        ...
        if (event_flag) {
          ...
          // (an incentive to ditch the ImWchar buffer, see
          ...
          if (buf_dirty) {
            ...
            InputTextReconcileUndoStateAfterUserCallback(
                ...
                callback_data
                    .BufTextLen);  // FIXME: Move the rest of this block inside function and rename
  ...
  if (apply_new_text != NULL) {
    // We cannot test for 'backup_current_text_length != apply_new_text_length' here because we have
    // no guarantee that the size of our owned buffer matches the size of the string object held by
    // the user, and by design we allow InputText() to be used without any storage on user's side.
  ...
  if (render_cursor || render_selection) {
...

    {
      ...
      // In multi-line mode, we never exit the loop until all lines are counted, so add one extra to
      // the searches_remaining counter.
...
bool ImGui::TreeNodeBehavior(ImGuiID id,
  ...
  // default or move it to ImGuiStyle. Currently only supports 32 level deep and we are fine with (1
...
void ImGui::TreePop() {
  ...
  // Handle Left arrow to move to parent tree node (when ImGuiTreeNodeFlags_NavLeftJumpsBackHere is
  // enabled)
...
//   if your SetNextItemSelectionData() values are indices, you can obtain it from
...
// ImGuiChildFlags_FrameStyle flag for stylistic changes + displaying a label. Tip: To have a list
// filling the entire window width, use size.x = -FLT_MIN and pass an non-visible label e.g.
// "##empty" Tip: If your vertical size is calculated from an item count (e.g. 10 * item_height)
// consider adding a fractional part to facilitate seeing scrolling boundaries (e.g. 10.25 *
// item_height).
