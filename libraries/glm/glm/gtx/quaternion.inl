...
GLM_FUNC_QUALIFIER GLM_CONSTEXPR qua<T, Q> quat_identity() {
  return qua<T, Q>::wxyz(static_cast<T>(1), static_cast<T>(0), static_cast<T>(0),
                         static_cast<T>(0));
}
...
template <typename T, qualifier Q>
GLM_FUNC_QUALIFIER qua<T, Q> intermediate(qua<T, Q> const& prev,
                                          qua<T, Q> const& curr,
                                          qua<T, Q> const& next) {
...
GLM_FUNC_QUALIFIER qua<T, Q> shortMix(qua<T, Q> const& x, qua<T, Q> const& y, T const& a) {
  ...
  return qua<T, Q>::wxyz(k0 * x.w + k1 * y2.w, k0 * x.x + k1 * y2.x, k0 * x.y + k1 * y2.y,
                         k0 * x.z + k1 * y2.z);
}
...
GLM_FUNC_QUALIFIER qua<T, Q> rotation(vec<3, T, Q> const& orig, vec<3, T, Q> const& dest) {
  ...
  return qua<T, Q>::wxyz(s * static_cast<T>(0.5f), rotationAxis.x * invs, rotationAxis.y * invs,
                         rotationAxis.z * invs);
}
}  // namespace glm
