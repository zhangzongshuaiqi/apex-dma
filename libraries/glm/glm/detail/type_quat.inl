...
struct compute_quat_mul_vec4 {
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR static vec<4, T, Q> call(qua<T, Q> const& q,
                                                            vec<4, T, Q> const& v) {
...
template <typename T, qualifier Q>
GLM_FUNC_QUALIFIER GLM_CONSTEXPR T const& qua<T, Q>::operator[](
    typename qua<T, Q>::length_type i) const {
  GLM_ASSERT_LENGTH(i, this->length());
#ifdef GLM_FORCE_QUAT_DATA_WXYZ
  return (&w)[i];
#else
...
GLM_FUNC_QUALIFIER qua<T, Q>::qua(vec<3, T, Q> const& u, vec<3, T, Q> const& v) {
  ...
  if (real_part < static_cast<T>(1.e-6f) * norm_u_norm_v) {
    ...
    t = abs(u.x) > abs(u.z) ? vec<3, T, Q>(-u.y, u.x, static_cast<T>(0))
