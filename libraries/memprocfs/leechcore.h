...
typedef struct LC_CONFIG {
  ...
  CHAR szRemote[MAX_PATH];  // remote connection striLC_CONFIG_VERSIONng - see wiki for additional
                            // info.
...
_Success_(return != NULL) EXPORTED_FUNCTION HANDLE
    LcCreateEx(_Inout_ PLC_CONFIG pLcCreateConfig,
...
_Success_(return) EXPORTED_FUNCTION BOOL
    LcAllocScatter1(_In_ DWORD cMEMs, _Out_ PPMEM_SCATTER* pppMEMs);
 ...
 * -- return
 */
_Success_(return) EXPORTED_FUNCTION BOOL LcAllocScatter2(_In_ DWORD cbData,
 ...
 */
_Success_(return) EXPORTED_FUNCTION BOOL
    LcAllocScatter3(_Inout_updates_opt_(0x1000) PBYTE pbDataFirstPage,
 ...
 * -- ppMEMs
 */
EXPORTED_FUNCTION VOID LcReadScatter(_In_ HANDLE hLC,
                                     _In_ DWORD cMEMs,
 ...
 * -- ppMEMs
 */
EXPORTED_FUNCTION VOID LcWriteScatter(_In_ HANDLE hLC,
                                      _In_ DWORD cMEMs,
                                      _Inout_ PPMEM_SCATTER ppMEMs);
 ...
 */
_Success_(return) EXPORTED_FUNCTION BOOL
    LcGetOption(_In_ HANDLE hLC, _In_ QWORD fOption, _Out_ PQWORD pqwValue);
 ...
 */
_Success_(return) EXPORTED_FUNCTION BOOL
    LcSetOption(_In_ HANDLE hLC, _In_ QWORD fOption, _In_ QWORD qwValue);
 ...
 * -- pcbDataOut
 */
_Success_(return) EXPORTED_FUNCTION BOOL LcCommand(_In_ HANDLE hLC,
                                                   _In_ QWORD fCommand,
                                                   _In_ DWORD cbDataIn,
...
#define LC_OPT_FPGA_ALGO_SYNCHRONOUS \
  0x0300008500000000  // RW - 1/0 use synchronous (old) read algorithm.
#define LC_OPT_FPGA_CFGSPACE_XILINX \
  0x0300008600000000  // RW - [lo-dword: register address in bytes] [bytes: 0-3: data, 4-7:
                      // byte_enable(if wr/set); top bit = cfg_mgmt_wr_rw1c_as_rw]
...
#define LC_OPT_FPGA_TLP_READ_CB_FILTERCPL \
  0x0300009100000000  // RW - 1/0 call TLP read callback with memory read completions from read
                      // calls filtered
#define LC_OPT_FPGA_TLP_READ_CB_BACKGROUND_THREAD \
  0x0300009200000000  // RW - 1/0 call TLP read callback auto-read with background thread [requires
...
#define LC_CMD_FPGA_CFGREGCFG_MARKWR \
  0x0000010700000000  // W  - write with mask [lo-dword: register address] [bytes: 0-1: data, 2-3:
                      // mask]
#define LC_CMD_FPGA_CFGREGPCIE_MARKWR \
  0x0000010800000000  // W  - write with mask [lo-dword: register address] [bytes: 0-1: data, 2-3:
                      // mask]
...
#define LC_CMD_FPGA_CFGSPACE_SHADOW_WR \
  0x0000010d00000000  // W  - [lo-dword: config space write base address]
...
static LPCSTR LC_STATISTICS_NAME[] = {
    "LcOpen",         "LcRead",      "LcReadScatter", "LcWrite",
    "LcWriteScatter", "LcGetOption", "LcSetOption",   "LcCommand",
};
 ...
 */
typedef VOID (*PLC_TLP_READ_FUNCTION_CALLBACK)(_In_opt_ PVOID ctx,
                                               _In_ DWORD cbTlp,
                                               _In_ PBYTE pbTlp,
